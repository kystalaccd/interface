...
/* 线程管理相关的全局变量 */
extern atomic_bool _running;    //接口是否运行的标志
extern mutex _config_interface_conn_mtx;
extern condition_variable _config_interface_conn;
...
class configInterface{
    ...
    friend class configure_factory;
    /*网卡信息*/
...
private:
    ...
    static int _setNet(const string& netinfo, const string& netPath, CJsonObject& outinfo);
    static int _getNet(vector<netInformation>& netinfo);
    static int _NetToJson(const struct netInformation& netinfo, string& netJson);
    static void _net_take_effect(CJsonObject data);
...
class configure_factory{
public:
    static configInterface* create(const string& ip, short port, const char*fmt/* 格式: %s %s (用空格分隔)*/, .../* 请按照enum EVENT的顺序输入文件路径 */){
        map<string, configInterface::paramType> uriEvent{    //默认map
            {GBURI/* 修改gbdevice配置文件对应的uri */, {-1/* 待写入的文件 */, _DEFAULT_GBCONFIG_PATH/* 配置文件的默认路径 */}},
            {NETURI/* 修改网络配置对应的uri */, {-1, _DEFAULT_NETCONFIG_PATH}}
        };

        if(fmt==NULL){
            configInterface* instance=new configInterface(uriEvent, ip, port);
            return instance;
        }
...
#endif
                struct configInterface::paramType temp{-1, string(str+slow, str+fast)};
#ifdef __DEBUG_CZX_
